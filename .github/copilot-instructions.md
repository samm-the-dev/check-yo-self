...
## Project-Specific: Check Yo Self

### Architecture

- Single-user PWA -- YNAB is the system of record, CYS reads from it
- Dexie IndexedDB for cache + check-in log only (no financial data stored)
- DailyBudget is derived at render time, never persisted
- Dark mode default, class-based toggle

### Do Flag

- YNAB API token exposure (must stay in localStorage, never in logs or network requests beyond YNAB)
- Dexie query results used without undefined checks (noUncheckedIndexedAccess is enabled)
- Milliunit conversion errors (YNAB uses milliunits: 1000 = $1.00)

### Do Not Flag

- localStorage for token storage (intentional for single-user PWA)
- Missing server-side validation (there is no server)
